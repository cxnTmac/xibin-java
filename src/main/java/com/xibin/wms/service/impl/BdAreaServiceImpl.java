...
public class BdAreaServiceImpl extends BaseManagerImpl implements BdAreaService {
	...
	public BdArea saveArea(BdArea model) throws BusinessException{
		...
		List<BdArea> list = bdAreaMapper.selectByKey(model.getAreaCode(),model.getCompanyId().toString(),model.getWarehouseId().toString());
	...
	public List<BdArea> selectByKey(String areaCode) {
		// TODO Auto-generated method stub
		UserDetails userDetails = (UserDetails)session.getAttribute(Constants.SESSION_USER_KEY);
		return bdAreaMapper.selectByKey(areaCode,userDetails.getCompanyId().toString(),userDetails.getWarehouseId().toString());
	}
